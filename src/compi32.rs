...
impl Visitor<Tree<Token>, Vec<String>, Tree<String>> for Compiler {
    fn visit_root(&mut self, expr: &Tree<Token>) -> Tree<String> {
        let name = Tree {
    ...
    fn visit(&mut self, expr: &Tree<Token>) -> Vec<String> {
        match expr.value.tok_type {
            ...
            TokenType::Op => {
                let mut res = Vec::new();
                res.append(&mut self.visit(&expr.children[0]));
                res.append(&mut self.visit(&expr.children[1]));
